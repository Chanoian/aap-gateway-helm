...
| Tag | Description |
|-----|-------------|
| `1.3.4` | Exact patch version — use this for pinned, reproducible installs |
...
| controller.secret_key_secret | string | `""` | Name of an existing Secret with key `secret_key` holding the controller's Django SECRET_KEY. Operator generates one if omitted. |
...
| secretProvider | object | `{"type":"","vso":{"auth":{"appRole":{"mount":"approle","roleId":"","secretRef":""}},"connection":{"address":"","caCertSecretRef":"","skipTLSVerify":false,"vaultNamespace":""},"kvVersion":"v2","refreshAfter":"1h","secrets":{"admin_password_secret":{"mount":"","path":"","refreshAfter":""},"controller_postgres_configuration_secret":{"mount":"","path":"","refreshAfter":""},"controller_secret_key_secret":{"mount":"","path":"","refreshAfter":""},"database_secret":{"mount":"","path":"","refreshAfter":""},"db_fields_encryption_secret":{"mount":"","path":"","refreshAfter":""},"eda_database_secret":{"mount":"","path":"","refreshAfter":""},"eda_db_fields_encryption_secret":{"mount":"","path":"","refreshAfter":""}},"vaultProject":""}}` | Secret provider plugin. Set `type` to activate a provider; leave empty to disable. Supported types: `vso` (Vault Secrets Operator). Each provider renders its own CRs — existing AAP config is untouched. |
...
| secretProvider.vso.vaultProject | string | `""` | Vault project name prepended to every secret's path (e.g. "aap" → "aap/admin"). |
